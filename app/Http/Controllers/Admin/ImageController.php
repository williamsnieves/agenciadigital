...
class ImageController extends Controller {
	...
	public function store(Request $request)
	{
		...
		//die(basename($request->input('url')));
		$image = new CustomImage;
		//$image->name = $request->input('name');
		$image->url = $request->input('url');
		$name = basename($request->input('url'));
		$image->name =  $name;


		$url = $request->input('url');
		$thumbnail = $request->input('thumbnail');
		$thumbnailProduct = $request->input('thumbnail_product');
		if(!empty($thumbnail) || $thumbnail  == 'on'){
			$image->isThumbnail = 1;
		...
		if(!empty($thumbnailProduct) || $thumbnailProduct  == 'on'){
